...
function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const moodImprovements = mockRecentMoods.map(mood => mood.moodAfter - mood.moodBefore);
      const averageImprovement = moodImprovements.reduce((a, b) => a + b, 0) / moodImprovements.length;
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold text-blue-600">{weeklyStats.sessionsCompleted}</p>
        </div>
        
        <div className="bg-white p-4 rounded-lg shadow">
          <h3 className="text-lg font-semibold mb-2">Sessions This Week</h3>
          <p className="text-3xl font-bold text-green-600">
            {weeklyStats.averageMoodImprovement > 0 ? '+' : ''}{weeklyStats.averageMoodImprovement}
          </p>
        </div>
        
        <div className="bg-white p-4 rounded-lg shadow">
          <h3 className="text-lg font-semibold mb-2">Total Practice Time</h3>
          <p className="text-3xl font-bold text-purple-600">{weeklyStats.totalPracticeTime}min</p>
        </div>
      </div>
            ...
            {recentMoods.map((mood, index) => (
              <div key={index} className="flex items-center justify-between border-b pb-2">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <span>→</span>
                  <span className={mood.moodAfter > mood.moodBefore ? 'text-green-600' : 'text-red-600'}>
