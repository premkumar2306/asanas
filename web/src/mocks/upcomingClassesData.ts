    ...
    if (recurringDays.includes(weekday)) {
      ...
      if (!existingTimestamps.has(startTime.getTime())) {
        ...
        repeatedClasses.push({
          ...
          time: startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' IST',
          duration,
          recurringDays,
...
export const generateDummyClasses = (): ClassItem[] => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    for (let j = 1; j <= classesPerDay; j++) {
      ...
      const type = j % 4 === 0 ? "workshop" : j % 3 === 0 ? "personal" : "group";
      const isRecurring = type === "group" || (type === "personal" && Math.random() > 0.5);

      const repeated = createRepeatedClasses(
        new Date(currentDate),
        isRecurring ? getRandomDays() : [currentDate.toLocaleDateString('en-US', { weekday: 'long' })],
        60,
        {
